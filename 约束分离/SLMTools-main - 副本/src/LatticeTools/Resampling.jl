module Resampling
using ..LatticeCore
using Interpolations: Flat, Periodic, interpolate, extrapolate, scale, BSpline, Cubic, OnGrid
using FFTW: fftshift, ifftshift

export downsample, coarsen, upsample

function cubic_spline_interpolation(Lu, x; extrapolation_bc=zero(eltype(x)))
    itp = interpolate(x, BSpline(Cubic(Flat(OnGrid()))))
    sitp = scale(itp, Lu...)
    etp = extrapolate(sitp, extrapolation_bc)
    return etp
end
